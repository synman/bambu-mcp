...
## What works on an H2D (fw 01.03.00.00, measured 2026-09-19)

| Operation | External (USB stick) | Internal (eMMC cache) |
|---|---|---|
...
| Rename | no command; download, upload new name, delete old | not possible |

Reads and deletes on internal have no eMMC root on this firmware, by path, name, `type`, `storage`, URL scheme or handshake field. Bambu's own cache wiki lists the H2 series as "Under Development" and the H2D reports `fun2` bit 17 = 0, so an unfinished feature is the likely reason. Official H2D firmware 01.04.00.00 (2026-08-26) is out and untested here.

## Tunnel versus FTPS on USB

For models the tunnel replaces FTPS on H2-series and P2S: list, download, upload, overwrite, delete, and it adds `SUB_FILE` member reads. It does not replace FTPS for everything. `LIST_INFO` returns only indexed `.gcode.3mf` models, so other file types and folders are invisible, the tunnel has no folder create or delete, and it has no rename (bpm's FTPS `move_file` renames).

## Tracking the server from MQTT

Read these from the printer's MQTT `push_status` (hex strings, bits counted from the least significant end). Values are for the H2D on fw 01.03.00.00.

| Field | Bit | Meaning | H2D |
|---|---|---|---|
| `fun` | 31 | the tunnel and upload protocol exists (`is_support_brtc`) | 1 |
| `fun2` | 0 | print from the eMMC cache is supported | 1 |
| `fun2` | 17 | Studio's Model Internal tab (the likely gate for internal browsing and reads) | **0** |
| `fun` | 28 | internal timelapse storage | 0 |
| `fun` | 49 | part skip, the feature that fetches `mem:/16` | 1 |

No live field says what the tunnel will serve, so bit 31 only proves it exists. Watch `fun2` bit 17 after every firmware upgrade and re-run the Verification block when it changes. bpm reads `fun` today but decodes none of these bits. Detail: `kb_get("bambu-mqtt-storage-capability-fields")`.
...
- Printer write protection (`bambu-ecosystem.md`) still applies: this client's helpers send only `LIST_INFO` (1) and `FILE_DOWNLOAD` (4), and `call` also sends `SUB_FILE` (2). `call` is a raw passthrough, so read-only holds by discipline: send only cmdtypes 1, 2, 4 and the read-only ability query 7.
  Do **not** send `FILE_UPLOAD`, `FILE_DEL` or `TASK_CANCEL` without the operator's explicit permission in the current turn, and never send an unknown cmdtype (it hangs, and its effect is unknown).
...
`cmdtype`: 1 `LIST_INFO`, 2 `SUB_FILE`, 3 `FILE_DEL`, 4 `FILE_DOWNLOAD`, 5 `FILE_UPLOAD`, 7 `REQUEST_MEDIA_ABILITY`, `0x1000` `TASK_CANCEL`. There is no rename, and an unknown cmdtype gets no reply.
`FILE_DOWNLOAD` also accepts `mem:/16`, which returns members of the printer's current or last job project (`project_member` below).
...
## Writes (operator-authorised only)

Not part of the reference client. Ask the operator first, every time, and test on a throwaway file. Measured 2026-09-19 on an H2D with `zz_mcp_*` files.
...
def upload(t, name, data, storage="udisk"):
    ...
    t.cmd_seq += 1
    seq = t.cmd_seq
    t._send(MAGIC_CTRL, json.dumps({"mtype": 12289, "cmdtype": 5, "sequence": seq,
            "req": {"type": "model", "storage": storage, "path": name, "total": len(data)}}).encode())
    _magic, payload = t._recv()
    init = json.loads(payload.partition(b"\n\n")[0])
    if init["result"] not in (1, 19):                          # 1 = new, 19 = exists, both proceed
        raise TunnelError(f"upload init result {init['result']}")
    assert len(data) <= init["reply"]["chunk_size"] * 1024, "multi-fragment upload is not measured"
    frag = {"mtype": 12289, "cmdtype": 5, "sequence": seq,
            "req": {"frag_id": 0, "offset": 0, "size": len(data), "file_md5": hashlib.md5(data).hexdigest()}}
    t._send(MAGIC_CTRL, json.dumps(frag).encode() + b"\n\n" + data)
    _magic, payload = t._recv()
    result = json.loads(payload.partition(b"\n\n")[0])["result"]
    if result != 0:
        raise TunnelError(f"upload result {result}")

def delete_usb(t, name):
    """Delete a USB file by full path, then re-list, because FILE_DEL reports result 0 even when nothing was removed."""
    t.call(3, {"paths": ["/media/usb0/" + name]})
    return all(e["name"] != name for e in t.list_files())

# upload(t, "zz_test.gcode.3mf", data)           -> appears at /media/usb0/zz_test.gcode.3mf, downloads back identical
# delete_usb(t, "zz_test.gcode.3mf")             -> True when it is gone
# move = download the old path, upload(t, new_name, data), delete_usb(t, old_name)
```
...
Not established: what `dir_refresh_cnt` means (always 0); printer-initiated notification payloads; what `api_version` 3 adds; other `mem:/N` indexes, and why the current project's gcode is refused; anything
about `TASK_CANCEL`, multi-fragment uploads, overwrite on internal, or a working internal read or delete on this firmware. Studio's five-member `zip:true` request works on USB (15,551 bytes, archive not opened). Other models differ: X2D reportedly allows eMMC downloads.

## Keeping this page current

- **Update when:** the H2D/P2S/X2D firmware changes (01.04.00.00 is out; the H2D here is on 01.03.00.00) (re-run the verification above — a firmware that lifts the `/media/usb0` allowlist changes the "When to use" table); Bambu Studio's
...
- **Source anchors:** Studio `bambulab/BambuStudio` @ `77b9dd94d1e3c432d5e74a18ab8de146ccf3b7c7`; ClusterM `open-bamboo-networking` (also cited as `open-bambu-networking`) @ `6aca2cb`, `research/06.04-port-6000.md`.
