...
| has_auto_switch_filament_support | **set to True when has_ams is True** (AMS detected in push_status telemetry; derived from AMS presence, not from a dedicated telemetry bit) | auto_switch_filament PrintOption is supported. No home_flag support bit exists; requires AMS (AMS-hosted spools only). Value at home_flag bit 10. |
